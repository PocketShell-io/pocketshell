 ...
 * (issue #2635 2a). Qualified because a bare `Flow<Set<Long>>` binding would be
 * an anonymous type in the graph.
 */
@Qualifier
@Retention(AnnotationRetention.BINARY)
...
object AppModule {
    ...
    /**
     * The host list's "which host is warm" feed (issue #2635 2a).
     *
    ...
    @Provides
    @LiveHostIds
    // @JvmSuppressWildcards: the javac component round keys bindings off the
    // JAVA view of the type, and Kotlin maps `Flow<Set<Long>>` to
    // `Flow<? extends Set<Long>>` there — an invariant provider key against a
    // wildcard request key reads as MissingBinding. Keep both sides the same.
    fun provideLiveHostIds(connections: ConnectionsRegistry): Flow<@JvmSuppressWildcards Set<Long>> =
