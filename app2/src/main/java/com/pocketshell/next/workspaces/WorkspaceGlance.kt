...
/**
 * The single-line workspace summary: a session count and a relative activity
 * label, sized to sit inside a 48dp row (issue #2635, D1 remainder).
 *
 * Replaces the wrapped per-kind `SessionKindSummary` subtitle on the workspace
 * list. The maintainer's reference is PocketShell Desktop's sidebar —
 * `pocketshell  4  now` — one line carrying name, count and recency, rather
 * than a title row plus a wrapped "Terminal ×2 · Claude" line underneath.
 *
 * The count is a BARE muted integer, not a filled chip: the desktop sidebar
 * this row copies uses a plain number, a filled surface on EVERY row is the
 * one bit of chrome a scannable list does not need, and the chip's 6dp radius
 * was off the radius ladder as well.
 *
...
fun WorkspaceGlance(
    ...
    Row(
        ...
        // A zero count renders nothing rather than a "0" badge: an empty
