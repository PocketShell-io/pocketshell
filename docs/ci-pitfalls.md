...
2. `grep -q` exits **the instant it matches**. The match is near the end, so grep leaves
   while `printf` still has bytes queued; `printf` takes SIGPIPE and the pipeline's status
   becomes **141**.
3. `pipefail` promotes that 141 to the pipeline's status, and the leading `!` turns
   "grep matched" into **"the guard failed."**
