...
| `standardRowMinHeight` | alias of `rowMinHeight` (56dp) | The standard row's reading height — an alias, never a second value (#2630) |
| `fieldMin` | `tokens.json` `size.fieldMin` (56dp) | Input/field minimum height (#2747 — composer draft editor floor, and every search field) |
| `buttonMin` | `tokens.json` `size.buttonMin` (56dp) | Button minimum height, kept even when a button renders compact in a banner or dialog row (#2800) |
| `workspaceRowMinHeight` | `tokens.json` `size.workspaceRowMin` (64dp) | Workspace row navigation target |
| `icon` | `tokens.json` `size.icon` (24dp) | The standard icon/glyph box — an `Icon` that is the row's or button's primary affordance (#2800) |
| `metadataIcon` | `tokens.json` `size.metadataIcon` (18dp) | The smaller glyph beside metadata text: kebab items, sheet headers, session-kind marks (#2800) |
| `rowPadV` | 16dp | Row vertical padding |
| `screenGutter` | `tokens.json` `size.screenGutter` (20dp) | Row horizontal padding and every screen's page inset (the screen gutter rung) |
