...
make_worktree() {
  ...
  # Issue #2712: the #2556 fixture-vintage gate reads the pins contract from
  # the CHECKOUT UNDER TEST. A wrapper-only sandbox has no pins file and dies
  # fail-closed at the gate ("no PIN lines ...") before the output-tree lock
  # this harness exists to exercise is ever reached. Copy the real file so
  # every lane here runs the gate's full comparison -- the gate is never
  # skipped, weakened, or stubbed out. The container side of the contract is
  # served by the docker stub below (issue #2712).
  cp "$ROOT_DIR/tests/docker/fixture-pins.txt" "$root/tests/docker/fixture-pins.txt"
...
exit 1
SYSTEMCTL_STUB
  # Issue #2712: with ANY docker on PATH -- a stub counts -- the connected
  # wrapper runs the #2556 fixture-vintage gate, which execs the port-2222
  # fixture container and reads its build-time-baked pins. The stub emulates
  # exactly that one interaction: a pocketshell-test-agents container (the
  # static 2222 name, scripts/lib/agents-pool.sh) whose
  # /opt/pocketshell-fixture/pins.txt was baked from THIS checkout's
  # tests/docker/fixture-pins.txt -- what a real `docker compose up -d --build`
  # from this checkout produces. Not a gate bypass: the gate still greps the
  # sandbox checkout's OWN pins file and diffs it against this answer, so a
  # sandbox whose copied pins drifted (or lost the file) still fails closed
  # before instrumentation -- proven by
  # fixture_vintage_gate_stays_fail_closed_in_harness_lanes below. An unset
  # HARNESS_FIXTURE_PINS makes the stub die loudly, which the gate reports as
  # a mismatch: this provision can only ever fail CLOSED.
  cat > "$bindir/docker" <<'DOCKER_STUB'
#!/usr/bin/env bash
set -uo pipefail
if [[ "${1:-}" == "exec" && "${3:-}" == "cat" \
      && "${4:-}" == "/opt/pocketshell-fixture/pins.txt" ]]; then
  cat "${HARNESS_FIXTURE_PINS:?HARNESS_FIXTURE_PINS must name the checkout tests/docker/fixture-pins.txt}"
  exit 0
fi
exit 0
DOCKER_STUB
...
# --------------------------------------------------------------------------
# The fixture-vintage gate (issue #2556) must stay LIVE and fail-closed in
# every harness lane. The happy-path checks now pass THROUGH the gate (the
# docker stub serves the checkout's own pins); this check proves the gate
# also still REFUSES. The stub's baked-pins provision (issue #2712) emulates
# a fixture built from this checkout; it must not double as a bypass: a
# sandbox checkout whose pins file DRIFTED from what the stub serves, or
# LOST the file entirely, must die at the gate before any gradle invocation
# -- otherwise this harness would quietly teach its lanes that the vintage
# contract is optional, the exact masking shape acceptance 3 of issue #2712
# forbids.
# --------------------------------------------------------------------------
fixture_vintage_gate_stays_fail_closed_in_harness_lanes() {
  local tmp="$1"
  local tag wt state log pins rc
  # The stub is a quoted heredoc INSIDE this file, so `bash -n` on the harness
  # never parses it -- an unbalanced quote in the ${:?} message made the stub
  # die at parse time (rc 2, no output) and every lane read that as an empty
  # fixture answer. Parse the INSTALLED stub directly.
  if ! bash -n "$STUB_BIN/docker" 2> "$tmp/vintage-stub-syntax.err"; then
    fail "the harness docker stub does not parse: $(cat "$tmp/vintage-stub-syntax.err")"
    return 1
  fi
  for tag in drifted missing; do
    wt="$tmp/wt-vintage-$tag"
    make_worktree "$wt"
    state="$tmp/state-vintage-$tag"
    new_state_dir "$state"
    log="$tmp/vintage-$tag.out"
    pins="$wt/tests/docker/fixture-pins.txt"
    if [[ "$tag" == "drifted" ]]; then
      # Only the SANDBOX copy drifts; HARNESS_FIXTURE_PINS (what the stub
      # serves) stays the real checkout file -- precisely the stale-fixture
      # shape #2556 refuses.
      sed -i 's/^APLEXER_PIN=.*/APLEXER_PIN=9.9.9-harness-drift/' "$pins"
    else
      rm -f "$pins"
    fi
    rc=0
    env \
      ...
      POCKETSHELL_AVD_LOCK_DIR="$AVD_LOCK_DIR" \
      POCKETSHELL_GRADLE_OUTPUT_LOCK_DIR="$OUTPUT_LOCK_DIR" \
      "$wt/scripts/connected-test.sh" --suffix "i2712$tag" \
      -Pandroid.testInstrumentationRunnerArguments.class=com.pocketshell.app.proof.HarnessProbe \
      > "$log" 2>&1 || rc=$?

    if [[ "$rc" == "0" ]]; then
      fail "[$tag pins] the lane exited 0 although its fixture vintage cannot match the checkout; the #2556 gate is being masked inside this harness (issue #2712). Lane log:
$(tail -n 20 "$log" | sed 's/^/      /')"
      return 1
    fi
    if grep -q ' invoked$' "$state/gradlew.log" 2>/dev/null; then
      fail "[$tag pins] the lane invoked gradle although the fixture-vintage gate must refuse it BEFORE any instrumentation (issue #2712). Lane log:
$(tail -n 20 "$log" | sed 's/^/      /')"
      return 1
    fi
    case "$tag" in
      drifted)
        if ! grep -qF 'AGENTS FIXTURE STALE' "$log"; then
          fail "[$tag pins] expected the #2556 stale-fixture banner for a drifted pins file, got:
$(tail -n 20 "$log" | sed 's/^/      /')"
          return 1
        fi
        # The refusal must be for the RIGHT reason: the banner's baked-pins
        # section must show the stub-served REAL checkout pin (0.1.5), which
        # can only come from the provision actually serving -- if the stub
        # died or answered empty, this check would pass vacuously on <none>.
        if ! grep -qF 'APLEXER_PIN=0.1.5' "$log"; then
          fail "[$tag pins] the stale banner shows no baked APLEXER_PIN=0.1.5, so the stub did not serve the checkout pins and the refusal is for the wrong reason. Lane log:
$(tail -n 20 "$log" | sed 's/^/      /')"
          return 1
        fi
        if grep -qF '9.9.9-harness-drift' "$log" && ! grep -qF '9.9.9-harness-drift' <(sed -n '/checkout pins/,/baked pins/p' "$log"); then
          fail "[$tag pins] the drifted pin did not appear in the checkout-pins section"
          return 1
        fi
        ;;
      missing)
        if ! grep -qF 'no PIN lines in' "$log"; then
          fail "[$tag pins] expected the missing-pins fail-closed diagnostic, got:
$(tail -n 20 "$log" | sed 's/^/      /')"
          return 1
        fi
        ;;
  ...
  pass "the fixture-vintage gate stays fail-closed in harness lanes (drifted and missing pins both refuse before gradle)"
}
...
main() {
  ...
  # The docker stub's baked-pins answer (issue #2712): the checkout this
  # harness runs from. make_worktree copies the same file into every sandbox,
  # so by construction the emulated fixture was built from the same pins the
  # sandbox under test carries -- and the fail-closed check below proves
  # drift or loss of that file still refuses.
  export HARNESS_FIXTURE_PINS="$ROOT_DIR/tests/docker/fixture-pins.txt"
  ...
  fixture_vintage_gate_stays_fail_closed_in_harness_lanes "$tmp" || true
