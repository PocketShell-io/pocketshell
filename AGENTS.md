...
Android SDK paths (may not be on PATH): `adb` at `/home/alexey/Android/Sdk/platform-tools/adb`, `emulator` at `/home/alexey/Android/Sdk/emulator/emulator`, SDK root `/home/alexey/Android/Sdk`, local AVD named `test`. Try these explicit paths before reporting emulator work as blocked. On `rewrite/js-first-0.6.0`, JS unit tests are `pnpm test:unit`; packaged Android lanes use `scripts/connected-test.sh <lane> --suffix i<issue>` (`smoke`, `lifecycle`, or `composer-docker`). The legacy JVM/app2 gates remain on `main`/`stable` pending #2863, and the rewrite has not yet replaced its 24-class/D36/D37 verdicts. Docker/port/runbook detail: [docs/docker-emulator-runbook.md](docs/docker-emulator-runbook.md).
