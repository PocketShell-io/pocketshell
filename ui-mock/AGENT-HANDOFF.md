...
- `.../hosts/` (updated 2026-09-18, D1): `HostListScreen.kt` split — the stateless `HostListScreen` composable plus the presentation types (`HostListUiState`, `HostRow`, `HostFormState`/`HostFormErrors`, `SshKeyRow`, `HostListUpdateNotice`) now live in the shared screen module `shared/ui-screens` (same package `com.pocketshell.next.hosts`, so call sites need no import churn); `HostListRoute.kt` and the ViewModels stay in app2 and keep the Room→row projection. No Room entities cross the boundary.
